@for($i = 0; $i < 3; $i++)
    @if (!empty($team[$i]))
        @php $medalTeam = $i > 0 ? $i == 2 ? 'third text-brown' : 'second text-secondary' : 'first text-warning' @endphp 
    @else
        @php $medalTeam = '' @endphp
    @endif

    @if (!empty($individual[$i]))
        @php $medalIndividual = $i > 0 ? $i == 2 ? 'third text-brown' : 'second text-secondary' : 'first text-warning' @endphp 
    @else
